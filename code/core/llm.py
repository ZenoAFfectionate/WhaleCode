...
class HelloAgentsLLM:
    ...
    def __init__(
        ...
        **kwargs
        ...
        # 验证必要参数
        if not self.model:
            raise HelloAgentsException("Model name is required (pass model= or set LLM_MODEL_ID)")
        if not self.api_key:
            raise HelloAgentsException("API key is required (pass api_key= or set LLM_API_KEY)")
        if not self.base_url:
            raise HelloAgentsException("Base URL is required (pass base_url= or set LLM_BASE_URL)")
