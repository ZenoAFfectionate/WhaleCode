...
class Config(BaseModel):
    ...
    # 上下文工程配置
    context_window: int = 262144  # 上下文窗口大小（tokens），匹配 Qwen3.5-35B-A3B
    compression_threshold: float = 0.7  # 压缩阈值（0.7 = 70%时触发 HistoryManager 压缩）
    ...
    compact_token_threshold: int = 225000  # 自动压缩触发阈值（~88% of 262k）
    compact_keep_recent_tool_results: int = 5  # 微压缩：保留最近 N 个工具结果完整内容
    compact_transcript_dir: str = "memory/transcripts"  # 完整转录保存目录
    compact_summary_max_tokens: int = 4096  # 压缩摘要最大 Token 数
